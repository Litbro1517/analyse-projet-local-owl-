...
# Import run_society from the owl.utils module (RolePlaying paradigm).
# Workforce paradigm modules (WORKFORCE_MODE=True) are handled
# directly via step() to avoid the 2-tuple interface mismatch.
try:
    from utils import run_society
    _HAS_RUN_SOCIETY = True
except ImportError:
    _HAS_RUN_SOCIETY = False
    logging.warning(
        "run_society not found in owl.utils; "
        "only Workforce-mode modules will be available."
    )
import datetime
...
def run_owl(question: str, example_module: str) -> Tuple[str, str, str]:
    ...
    try:
        ...
        # Detect module paradigm --------------------------------------------------
        # WORKFORCE_MODE flag (set at module level) tells us whether the
        # module uses the Workforce paradigm (direct step()) or the
        # RolePlaying paradigm (run_society with 2-tuple step()).
        is_workforce = getattr(module, "WORKFORCE_MODE", False)
        logging.info(
            f"Module paradigm detected: {'Workforce' if is_workforce else 'RolePlaying'}"
        )

        # Build society / workforce bridge
        try:
        ...
        # Execute the task ----------------------------------------------------
        try:
            # ── Pre-execution cooldown (2 s) to avoid RPM burst ───────────
            # Google AI Studio Free Tier has strict RPM limits.
            # This 2-second pause spreads the initial connection spike.
            logging.info("Applying 2-second pre-execution cooldown...")
            time.sleep(2)

            if is_workforce:
                # ── Workforce paradigm ─────────────────────────────────────
                # Workforce modules return a bridge object whose step()
                # method processes the entire task in a single call and
                # returns a single response (NOT the 2-tuple expected by
                # run_society).  We call step() directly and extract the
                # answer from response.msg.content.
                logging.info("Running workforce task...")
                society.init_chat()  # no-op for Workforce, but keeps API
                response = society.step()

                if response is not None:
                    # Extract answer from the response object
                    answer = (
                        getattr(response, "msg", None)
                        and getattr(response.msg, "content", None)
                    ) or getattr(response, "content", str(response))
                else:
                    answer = "No response from workforce."

                chat_history = []
                token_info = {}
                logging.info("Workforce task completed")

            else:
                # ── RolePlaying paradigm ───────────────────────────────────
                if not _HAS_RUN_SOCIETY:
                    return (
                        "run_society is not available. "
                        "This module requires the RolePlaying paradigm but "
                        "owl.utils.run_society could not be imported.",
                        "0",
            ...
            logging.error(f"Error occurred while running task: {str(e)}")
            return (
                f"Error occurred while running task: {str(e)}",
                "0",
